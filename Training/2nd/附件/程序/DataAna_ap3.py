...
sns.regplot(x='budget',y='revenue',data=df_mcap)
...
for key in type_num.keys():
    type_ratio[key]=type_num[key][0]/type_num[key][1]
stn1=sorted(type_ratio.items(), key = lambda kv:(kv[1], kv[0]),reverse=True)
for i in stn1:
    print(i[0],i[1])
stn2=sorted(type_num.items(), key = lambda kv:(kv[1][0], kv[0]),reverse=True)
for i in stn2:
    print(i[0],i[1])
print("————————————————")
