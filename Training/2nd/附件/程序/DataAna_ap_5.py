...
for date in release_date:
    ...
    if y not in year:
        year.add(y)
for y in year:
    month_revenue[y]=[[] for i in range(12)]
for i in df_mcap.index:
    y=release_date[i].split('/')[0]
    m=int(release_date[i].split('/')[1])
    r=revenue[i]
    month_revenue[y][m-1].append(r)
month_revenue2=dict()
for y in year:
    month_revenue2[y]=[0 for i in range(12)]
for y in month_revenue:
    for i in range(len(month_revenue[y])):
        if len(month_revenue[y][i])!=0:
            month_revenue2[y][i]=sum(month_revenue[y][i])/len(month_revenue[y][i])
        else:
            month_revenue2[y][i]=0

out=sorted(month_revenue2.items(), key = lambda kv:(int(kv[0]), kv[1]),reverse=False)
print(out)


df=pd.DataFrame([i[1] for i in out],index=[i[0] for i in out],columns=month)
df.to_csv(".\\data\\tmdb_5000_revenue_month.csv",index=True,header=True)
df_st=pd.read_csv(".\\data\\tmdb_5000_revenue_month.csv")
'''
sns.pointplot(x='year',y='',hue=style,data=df_st)
'''
plotdf12=df.loc[[i[0] for i in out],month]
