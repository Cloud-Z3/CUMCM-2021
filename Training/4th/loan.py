...
df_rate=pd.read_csv(".\\data\\风险量化与贷款利率.csv")
df_assess=pd.read_csv(".\\data\\风险量化与评级.csv")
...
for i in out:
    if state[i[0]]!='no loan':
        state[i[0]]=limit[i[0]]
for i in range(123):
    print('E'+str(i+1),'\t',assess[i],'\t',state[i],'\t',r[i])
dfw=pd.DataFrame([['E'+str(i+1),assess[i],'是' if state[i]!='no loan' else '否',state[i] if state[i]!='no loan' else '-',
                   r[i] if state[i]!='no loan' else '-'] for i in range(123)],columns=['企业代号','计算信用评级','是否贷款','贷款金额','贷款利率'])
dfw.to_csv('.\\data\\问题一求解结果.csv',index=False)