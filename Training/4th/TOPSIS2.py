...
def topsis(weight):
    df_ap=pd.read_csv('.\\data\\afterPreProcesss2.csv')
    ...
    for i in df_ap.index:
        ...
        sort[i]=sort1[i]/(sort1[i]+sort2[i])

    out=sorted(sort.items(), key = lambda kv:(kv[1], kv[0]),reverse=False)
    print(out)
    assign=[0 for i in range(302)]
    counter=0
    index=0
    index2=0
    while(index<4):
    ...
    for i in range(len(df_ap)):
        ...
        ...
    dfw=pd.DataFrame([['E'+str(i+1),'%.6f'%sort[i],assign[i]] for i in range(302)],columns=['企业代号','风险量化','风险评级'])
    dfw.to_csv('.\\data\\风险量化与评级2.csv',index=False)
    '''
    for i in range(123):
    ...
    plt.show()
    return count/302,spearmanr(credit,assign)
'''
