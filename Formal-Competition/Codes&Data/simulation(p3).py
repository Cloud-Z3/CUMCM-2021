...
def simulation(k):
    ...
    while week<24:
        ...
        for si in range(s_index,402):
            ...
            aveOrder[int(rank[si][1:]) - 1][week] = None
        s_index=0
        total+=28200
        week+=1
        #print(total)
...
aveOrder=[[None for i in range(24)] for j in range(402)]    # 周平均订货量
...
transfer=[[None for i in range(8*24)] for j in range(402)]                                 #供应限制
...
dfwT.to_csv('.\\data\\simulation&check(p3)\\simulationTransfer.csv')
dfwOT=pd.DataFrame(Ordertransfer,columns=['W' + (3 - len(str(j))) * str(0) + str(j) for j in range(1, 25)],index=df_order['供应商ID'])
dfwOT.to_csv('.\\data\\simulation&check(p3)\\simulationOrderTransfer.csv')
dfwsa=pd.DataFrame(satisfiction,columns=['W' + (3 - len(str(j))) * str(0) + str(j) for j in range(1, 25)],index=df_order['供应商ID'])
dfwsa.to_csv('.\\data\\simulation&check(p3)\\satisfiction.csv')'''

print(storage)
plt.plot(list(range(0,25)),storage)
plt.plot([0,24],[2*28200,2*28200])
plt.axis([-1,25,min(storage)-500,max(storage)+500])
